...
Bigint operator*(const Bigint& a, const Bigint& b) {
    ...
    c.s.resize(n + 1, 0);
    for (LL i = 0; i < na; ++i)
        for (LL j = 0; j < nb; ++j) c.s[i + j] += a.s[i] * b.s[j];
    for (LL i = 0; i < n; ++i) {
        c.s[i + 1] += c.s[i] / c.BASE;
        c.s[i] %= c.BASE;
    }
...
struct Point {
    LL a, b;
    bool operator<(const Point& rhs) const { return a * b < rhs.a * rhs.b; }
} p[maxn];
int main() {
    cin >> n >> p[0].a >> p[0].b;
    for (int i = 1; i <= n; ++i) cin >> p[i].a >> p[i].b;
    sort(p + 1, p + 1 + n);
    Bigint ans = 0, dot = p[0].a;
    for (int i = 1; i <= n; ++i) {
        Bigint now = dot / p[i].b;
        if (now > ans) ans = now;
        dot = dot * p[i].a;
    }
    cout << ans << endl;
